...
def freq(word, word_list):
    ans = 0
    for synonym in synonyms(word):
        ans += word_list.count(synonym)
    return ans
...
class BM25:
    ...
    def vect(self, docs, queries, save, cX, qX):
        print("Catching vectorised fis and qfis for faster calculations")
        a = []
        ...
        doc = {}
        for name, cont in tqdm(docs.items()):
            doc[name] = {"dl": len(cont)}
            for i in Is:
                doc[name][i] = freq(i, cont)
        ...
        query = {}
        for name, cont in tqdm(queries.items()):
            query[name] = {}
            for i in Is:
                query[name][i] = freq(i, cont)
